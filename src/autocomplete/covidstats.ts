...
import { objectFlip } from "../util/objects";

export default new AutoCompleter({
  command: "covidstats",
  execute({ ctx }) {
    const obj = objectFlip({
      AF: "Afghanistan",
      AL: "Albania",
      DZ: "Algeria",
      AS: "American Samoa",
    ...
    for (const country in obj) {
      if (Object.prototype.hasOwnProperty.call(obj, country)) {
        const e = obj[country];
        if (!country.toLowerCase().includes(ctx.options.getString("country").toLowerCase())) continue;
        response.push({
          name: country,
          value: e,
        });
    ...
    return response.slice(0,20); // 25 response cap
  },
});
