...
class Giveaway extends EventEmitter {
  ...
  async fetchMessage() {
    return new Promise(async (resolve, reject) => {
      ...
      const channel = await this.client.channels
        ...
        .catch((err) => {
          if (
            err.httpStatus.toString().startsWith("5") ||
            err.httpStatus === 429 ||
            err.code === 130000
          )
            tryLater = true;
      ...
      if (!message) {
        ...
        return reject(
          "Unable to fetch message with Id " + this.messageId + "." + tryLater
            ? " Try later!"
            : ""
        );
      }
  ...
  async roll(winnerCount = this.winnerCount) {
    ...
    // Fetch guild members
    if (
      new Discord.Intents(this.client.options.intents).has(
    ...
    if (!userArray || userArray.length <= winnerCount)
      rolledWinners = users.random(winnerCount);
    else {
      /**
  ...
  edit(options = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      if (
        !isNaN(options.addTime) &&
        typeof options.addTime === "number" &&
        !this.isDrop
      ) {
      ...
      if (
        !isNaN(options.setEndTimestamp) &&
        typeof options.setEndTimestamp === "number" &&
        !this.isDrop
      )
   ...
   */
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (this.isDrop || this.endAt < this.client.readyTimestamp)
        this.endAt = Date.now();
      await this.manager.editGiveaway(this.messageId, this.data);
      ...
      const channel =
        ...
        !this.message.channel
          ...
          ?.has([
            this.message.channel.locked ||
            (!this.message.channel.joined &&
      ...
      if (winners.length > 0) {
        ...
        if (
          ...
          const embedDescription = embed.description.replace(
            "{winners}",
            formattedWinners
          );
          if (embedDescription.length <= 4096) {
              ...
              embeds: [
                new Discord.MessageEmbed(embed).setDescription(
                  embed.description.substr(
                    0,
                    embed.description.indexOf("{winners}")
                  )
                ),
              ],
              allowedMentions: this.allowedMentions,
            ...
            channel.send({
              embeds: [
                tempEmbed.setDescription(
                  embed.description.substr(
                    embed.description.indexOf("{winners}") + 9
                  )
                ),
              ],
              allowedMentions: this.allowedMentions,
