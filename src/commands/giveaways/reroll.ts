...
export = class RerollGiveaway extends Command {
  name = "reroll";
  disabled? = false;
  description = "Reroll a giveaway";
  usage = "<message id>";
  aliases = ["rr", "rerollgiveaway"];
  args = true;
  ...
  data = new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setDescription(
          "Message ID of giveaway to reroll. Right click to copy ID!"
        )
        .setName("id")
        .setRequired(true)
    );
  ...
  execute = async (interaction: CommandInteraction) => {
    ...
    if (!repl) {
      ...
      return interaction.channel?.send("Cancelled Reroll!");
    }
    if (repl.customId === "confirm") {
      this.bot.giveawayManager
        ...
        .then((g) =>
          repl.reply(
            `${
              interaction.user
            }, the giveaway has been rerolled! New winners are:\n\n${
              g.join(", ") ||
              "Not enough participants! Winners cannot be determined..."
            }`
          )
        )
        .catch((err) => {
      ...
      interaction.editReply({ components: disabled });
      return repl.reply("Cancelled reroll!");
    }
  };
};
